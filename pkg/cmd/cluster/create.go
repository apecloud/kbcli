...
const (
	...
	keyMonitor      setKey = "disableExporter"
)
...
func setBackup(o *CreateOptions, cluster *appsv1alpha1.Cluster) error {
	backupName := o.Backup
	if len(backupName) == 0 {
		return nil
	}
	...
	// }
	restoreAnnotation, err := restore.GetRestoreFromBackupAnnotation(backup, o.VolumeRestorePolicy, restoreTimeStr, nil, false, nil)
	if err != nil {
		return err
	}
...
func (o *CreateOptions) Complete() error {
	...
	if err = setBackup(o, cls); err != nil {
		return err
	}
...
func (o *CreateOptions) buildComponents(clusterCompSpecs []appsv1alpha1.ClusterComponentSpec) ([]map[string]interface{}, error) {
	...
	if clusterCompSpecs != nil {
		setsCompSpecs, err := buildClusterComp(cd, compSets, o.DisableExporter, o.CreateOnlySet)
		if err != nil {
			return nil, err
		}
		...
		compSpecs, err = buildClusterComp(cd, compSets, o.DisableExporter, o.CreateOnlySet)
		if err != nil {
			return nil, err
		}
	}
...
func buildClusterComp(cd *appsv1alpha1.ClusterDefinition,
	...
	for i, c := range cd.Spec.ComponentDefs {
		...
		if monitor == "" {
			compObj.DisableExporter = cfgutil.ToPointer(disableExporter)
		} else {
			enabled, err := strconv.ParseBool(monitor)
			if err != nil {
				return nil, fmt.Errorf("parsing monitor failed: %s", err.Error())
			}
			compObj.DisableExporter = cfgutil.ToPointer(enabled)
		}
